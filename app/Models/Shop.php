...
class Shop extends Model
{
    ...
    public function content()
    {
        ...
        foreach ($content as $key=>$value) {
            switch ($key) {
                case "bandwidth":
                    $content_text .= "添加流量 ".$value."GB，";
                    break;
                case "expire":
                    $content_text .= "为账号的有效期添加 ".$value." 天 ";
                    break;
                case "class":
                    $content_text .= "可用线路 ".$value." 条，有效期 ".$content["class_expire"]." 天。";
                    break;
                case "reset":
                    $content_text .= "在 ".$content["reset_exp"]." 天内，每 ".$value." 天重置流量为 ".$content["reset_value"]." GB";
                    break;
                default:
            }
        ...
        foreach ($content as $key=>$value) {
            switch ($key) {
                ...
                case "class":
                    ...
                    break;
				case "reset_value":
					$user->auto_reset_day=date("d", time());
					$user->auto_reset_bandwidth=$content["reset_value"];
					break;
                default:
            }
        }
