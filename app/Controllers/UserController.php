                ...
                if ($node->sort==0||$node->sort==7||$node->sort==8||$node->sort==10) {
                    ...
                    if ($node->isNodeOnline() !== null) {
                        if ($node->isNodeOnline() === false) {
                            $node_heartbeat[$temp[0]]="<img src='/backend/img/off.png' style ='position:relative; top:-1px'> 离线中";
                        } else {
                            $node_heartbeat[$temp[0]]="<img src='/backend/img/on.png' style ='position:relative; top:-1px'> 服务中";
                        }
                    } else {
                        if (!isset($node_heartbeat[$temp[0]])) {
                            $node_heartbeat[$temp[0]]="<img src='/backend/img/over.png' style ='position:relative; top:-1px'> 维护中";
                        }
                    }
                    if ($node->node_bandwidth_limit==0) {
                        $node_bandwidth[$temp[0]]=(round)($node->node_bandwidth/1024/1024/1024,2)." GB";
                    } else {
                        $node_bandwidth[$temp[0]]=(round)($node->node_bandwidth/1024/1024/1024,2)." GB / ".(int)($node->node_bandwidth_limit/1024/1024/1024)." GB - ".$node->bandwidthlimit_resetday."日清零";
                    }
...
*/
    ...
    public function codepost($request, $response, $args)
    {
        ...
        if ($codeq->type==-1) {
            ...
            $res['ret'] = 1;
            $res['msg'] = "充值成功！充值金额为".$codeq->number."元。";
            if (Config::get('enable_donate') == 'true') {
                if ($this->user->is_hide == 1) {
                    Telegram::Send("客户无名氏给我们捐了".$codeq->number."元。");
                } else {
                    Telegram::Send("客户".$this->user->user_name."给我们捐了".$codeq->number."元。");
                }
            }
            return $response->getBody()->write(json_encode($res));
        }
...
*/
    public function updateWechat($request, $response, $args)
    {
        ...
        if ($user->telegram_id != 0) {
            $res['ret'] = 0;
            $res['msg'] = "您绑定了Telegram，所以此项并不能被修改！";
            return $response->getBody()->write(json_encode($res));
        }
        if ($wechat == ""||$type == "") {
            $res['ret'] = 0;
    ...
    public function updateSSR($request, $response, $args)
    {
        ...
        if (!Tools::checkNoneProtocol($user)) {
            $res['ret'] = 0;
            $res['msg'] = "您好！系统检测到您目前的加密方式为none，但您将要设置为的协议并不在以下协议<br />".implode(',', Config::getSupportParam('allow_none_protocol')).'<br />之内，请您先修改您的加密方式，再来修改此处设置！';
            return $this->echoJson($response, $res);
        }
        if(!URL::SSCanConnect($user) && !URL::SSRCanConnect($user)) {
            $res['ret'] = 0;
            $res['msg'] = "您好！您这样设置之后将导致无法连接，所以系统拒绝了您的修改请求，请您检查您的设置之后再进行操作！";
            return $this->echoJson($response, $res);
        }
        $user->save();
        if(!URL::SSCanConnect($user)) {
            $res['ret'] = 0;
            $res['msg'] = "设置成功！请您自行更换使用ShadowsocksR客户端！";
            return $this->echoJson($response, $res);
        }
        if(!URL::SSRCanConnect($user)) {
            $res['ret'] = 0;
            $res['msg'] = "设置成功！请您自行更换使用Shadowsocks客户端！";
            return $this->echoJson($response, $res);
        }
        $res['ret'] = 0;
        $res['msg'] = "设置成功！";
...
*/
    ...
    public function updateMethod($request, $response, $args)
    {
        ...
        if (!Tools::checkNoneProtocol($user)) {
            $res['ret'] = 0;
            $res['msg'] = "您好！系统检测到您将要设置的加密方式为none，但您的协议并不在以下协议<br />".implode(',', Config::getSupportParam('allow_none_protocol')).'<br />之内，请您先修改您的协议，再来修改此处设置！';
            return $this->echoJson($response, $res);
        }
        if(!URL::SSCanConnect($user) && !URL::SSRCanConnect($user)) {
            $res['ret'] = 0;
            $res['msg'] = "您好！您这样设置之后将导致无法连接，所以系统拒绝了您的请求，请您检查您的设置之后再进行操作！";
            return $this->echoJson($response, $res);
        }
